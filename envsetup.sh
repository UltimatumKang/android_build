...
- cmremote: Add git remote for CM Gerrit Review. NONONONO
- cmgerrit: A Git wrapper that fetches/pushes patch from/to CM Gerrit Review. NONO
- cmrebase: Rebase a Gerrit change and push it again.NAH
- aospremote: Add git remote for matching AOSP repository.
- mka:      Builds using SCHED_BATCH on all processors.
- mkap:     Builds the module(s) using mka and pushes them to the device.
- cmka:     Cleans and builds using mka.
- reposync: Parallel repo sync using ionice and SCHED_BATCH.
- repopick: Utility to fetch changes from Gerrit. I NO WANTS
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^ukg_") ; then
       UKG_BUILD=$(echo -n $1 | sed -e 's/^ukg_//g')
    else
       UKG_BUILD=
    fi
    export UKG_BUILD
...
function addcompletions()
{
    ...
    dirs="sdk/bash_completion vendor/ukg/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var UKG_VERSION)
        ZIPFILE=uXylon-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.ukg.device=$UKG_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $UKG_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $UKG_FIXUP_COMMON_OUT ]; then
